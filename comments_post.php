...
foreach ($comments_all as $value) {
    ...
    $post_id = $value['cid'];
    if ($post_info[$post_id]['type'] != "post") continue;
    $category = $post_info[$post_id]['category'];

    $url = "/" . $category . "/" . $post_id . ".html";
...
$file = fopen("./comments_post.json", "w");
fwrite($file, json_encode($results));
echo "All post's comments processed!";